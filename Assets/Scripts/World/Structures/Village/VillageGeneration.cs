...
public class VillageGeneration : MonoBehaviour
{
    ...
    public GameObject house, villageTree, villageHall;
    private GameObject villageTreeObject;
    ...
    public void Initialize(TileGrid g)
    {
        ...
        foreach (Village vil in villages)
        {
            ...
            // Spawn village tree (to be added later)
            villageTreeObject = Instantiate(villageTree, new Vector3(vilCenter.x+.5f, vilCenter.y+.5f), Quaternion.identity, transform);
            structures.Add(villageTreeObject);
            ...
            villageTreeObject.GetComponent<BoxCollider2D>().size = new Vector2(vilMaxWidth*2.3f, vilMaxHeight*2.7f);
        }
    ...
    private void SpawnVillageSquare(Vector3 centerPos)
    {
        ...
        var hall = Instantiate(villageHall, new Vector3(vilCenter.x+.5f, vilCenter.y+vilSquareRad+1.5f), Quaternion.identity, transform);
    ...
    private void SpawnHouse(Vector3 housePos)
    {
        if (grid.CheckLand(housePos) || grid.CheckCliff(housePos))
        {
            ...
            villageTreeObject.GetComponent<VillageData>().lots.Add(housePos);
        ...
        foreach(var letter in sequence)
        {
            ...
            switch (encoding)
            {
                ...
                case EncodingLetters.load:
                    ...
                    {
                        throw new Exception("Dont have save point in our stack");
                    }
                    break;
