...
public class VegetationGeneration : MonoBehaviour
{
    ...
    [SerializeField]
    private NoiseValues[] otherVegetation;
    ...
    public void GetVegetation(int x, int y, GameObject chu)
    {
        ...
        {
            // Loop over other vegetation
            for (int i=0; i<otherVegetation.Length; i++)
            {
                if (noiseHeight <= otherVegetation[i].height && random.Next(0, 100) <= otherVegetation[i].spawnChance)
                {
                    ...
                    if (grid.CheckLand(new Vector2(x, y)) && !villageNeighbors.ContainsValue((int)GroundTileType.VillagePath) && !villageNeighbors.ContainsValue((int)GroundTileType.VillagePlot))
                    {
                        ...
                        // Spawn vegetation
                        GameObject vege = Instantiate(otherVegetation[i].vegetationObject, new Vector3(x+randX, y+randY), Quaternion.identity, chu.transform);
                        if (cliffMap.GetTile((int)(x+randX), (int)(y+randY)) == (int)GroundTileType.Cliff)
                        {
                            vege.GetComponent<SpriteRenderer>().sortingOrder+=1;
                        }
