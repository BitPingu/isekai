...
public class NoiseGeneration : AlgorithmBase
{
    ...
    {
        // Order TileTypes from small to high
        TileTypes = TileTypes.OrderBy(a => a.Height).ToArray();

        // Pass parameters to generate noise
        ...
        if (ApplyIslandGradient)
        {
            ...
            for (int x=0, y; x<tilemap.Width; x++)
            {
                for (y=0; y<tilemap.Height; y++)
                {
                    ...
                    // Apply it into the map, but clamp between 0f and 1f
        ...
        for (int x=0; x<tilemap.Width; x++)
        {
            for (int y=0; y<tilemap.Height; y++)
            {
                ...
                {
                    // If the height is smaller or equal, use this tiletype
                    if (height <= TileTypes[i].Height)
                    {
                        if ((int)TileTypes[i].GroundTile == 2)
                        {
